...
function mapsHref(r: Restaurant): string {
  if (r.maps_url && r.maps_url.trim() !== '') return r.maps_url;
  const q = encodeURIComponent(`${r.name_jp} ${r.address}`.trim());
...
export default function RestaurantModal({ restaurant, userLocation, onClose }: Props) {
  ...
  const cuisineLabel = lang === 'en' ? cuisine.label_en : cuisine.label_zh;
  ...
  const reviewText =
    lang === 'en'
      ? `Based on ${restaurant.review_count.toLocaleString()} Google reviews`
      : `基于 ${restaurant.review_count.toLocaleString()} 条 Google 评价`;
  const updatedText =
    lang === 'en'
  ...
  const distanceChip =
    lang === 'en'
      ? `📍 approx. ${formatDistance(distanceKm ?? 0)}`
      : `📍 直线约 ${formatDistance(distanceKm ?? 0)}`;
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-5">
          ...
          <p className="mt-0.5 text-sm text-ink-500">{secondaryName}</p>
          ...
          <div className="mt-3 flex items-center gap-3">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-3 py-1 text-sm font-semibold text-amber-700">
              ...
              {restaurant.google_rating > 0 ? restaurant.google_rating.toFixed(1) : '—'}
            </span>
            <span className="text-sm text-ink-500">{reviewText}</span>
          </div>

          <p className="mt-4 text-[15px] leading-relaxed text-ink-700">{description}</p>
          ...
          <div className="mt-5 rounded-xl bg-ink-50 px-4 py-3">
            ...
            {distanceKm != null && (
              <div className="mt-1 text-[11px] text-ink-400">{t.modalDistanceNote}</div>
            )}
          </div>

          {/* CTA */}
          <a
            href={mapsHref(restaurant)}
            target="_blank"
          ...
          <p className="mt-3 text-center text-[11px] text-ink-400">{updatedText}</p>
        </div>
      </div>
    </div>
  );
